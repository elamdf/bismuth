...
The N prefix to CR or XCR marks a thread inactive,
removing it from the actionable list of both author and reviewer without deleting the text.

CR headers and threads may be optionally prefixed with a configurable comment string (see =comment-regex= in =inline-cr.el=) to facilitate usage outside of markup languages.
...
* TODOs
** TODO support non-markdown programming languages
This should just be a question of updating the regexes to allow whitespace/comment prefixes for a CR and maybe fixing some of the thread navigation helpers so toggles/thread extensions don't blow up the source code
