...
All navigation/aggregation functionality is implemented via commands (you can see what's available by doing Ctrl-p and typing "bismuth").
You can bind these commands to keys using Obsidian's hotkeys [[https://help.obsidian.md/hotkeys][support]].
...
- jump directly to actionable point (not just file) from aggregate note
- see actionables created by recent git commits
- more native window for aggregation with click-to-follow link, the discrete note is a bit clunky


* TODOs
** TODO A nice github-like visual aggregator for all open (i.e. existing) comment threads
** generic parser binary
.. probably in a faster langauge, so as to support other frontends than emac
